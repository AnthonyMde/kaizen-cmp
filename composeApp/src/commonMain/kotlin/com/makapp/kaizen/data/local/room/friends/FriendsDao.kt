...
interface FriendsDao {
    ...
    // To avoid to delete user challenges we look for stale challenges where userId is a friendId.
    @Query("DELETE FROM ChallengeEntity WHERE id NOT IN (:keptIds) AND userId IN (:friendIds)")
    suspend fun deleteStaleChallengeEntities(keptIds: List<String>, friendIds: List<String>)
    ...
    suspend fun refresh(friendsWithChallenges: List<FriendWithChallengesEntity>) {
        ...
        deleteStaleChallengeEntities(keptIds = challengesIds, friendIds)
    }
}
